...
class ForeGroundWidget extends StatefulWidget {
  const ForeGroundWidget( this.nextPrayer, this.reminderTime, this.currentPrayer, {Key? key}) : super(key: key);
...
class _ForeGroundWidgetState extends State<ForeGroundWidget> {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (prayer.selected) Container(width: 92, height: 2.3, color: const Color(0xFFE26B26))
        ],
      ),
    );
  }
  ...
  Widget loaderWidget() => SizedBox(
        ...
        child: FittedBox(
          child: CircularPercentIndicator(
            ...
            percent: progress(widget.currentPrayer?.time, widget.nextPrayer.time),
            center: insideLoader(),
            animation: !firstTime,
  ...
  Widget insideLoader() => Column(
        ...
        children: [
          ...
          Text(widget.nextPrayer.title, style: const TextStyle(color: Colors.white, fontSize: 18)) //Next prayer
        ],
      );
}
