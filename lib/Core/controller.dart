    ...
    if (prayerJs != null) {
      ...
      for (var element in (jsonDecode(prayerJs) as List<dynamic>)) {
        if (DateFormat("dd-MM-yyyy").parse(element["date"]).isSameDate(now)) {
          todayPrayer = (jsonDecode(element["prayers"]) as List<dynamic>)
              .map((e) => Prayer.fromJson(e))
              .toList();
        }
      }
    } else {
      ...
      DateTime now = DateTime.now();
      prefs.setString('prayers',
          jsonEncode(allDates, toEncodable: (e) => (e as PrayerDate).toJson()));
    ...
    getCountry().then((value) async {
      if (value['country'] != prefs.get('country')) {
        ...
        await prefs.setString(
            'prayers',
            jsonEncode(allDates,
                toEncodable: (e) => (e as PrayerDate).toJson()));
